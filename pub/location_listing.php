...
?>
<div id="<?php echo $css['xtreme_locator_search_results']['class']; ?>">
    <div id="<?php echo $css['xtreme_locator_search_locations']['class']; ?>">
		<?php
		...
		if ( $resultType == 3 ) {
			...
			$map_it  = "<div class='map_it'><a href='#' onClick=\"NewWindow('" . $gconf->domain . "/visitor/googlemap.php?lid=" . $row['_id'] . "&sid=" . $gconf->site_id . "&zoom=14&language=&info=1&width=" . ( isset( $_GET['lid'] ) ? $conf->map_width_details : $conf->map_width_list ) . "%&height=" . ( isset( $_GET['lid'] ) ? $conf->map_height_details : $conf->map_height_list ) . "%', '_blank', " . ( isset( $_GET['lid'] ) ? $conf->map_width_details : $conf->map_width_list ) . ", " . ( isset( $_GET['lid'] ) ? $conf->map_height_details : $conf->map_height_list ) . ", 0);return false;\"><img src='" . XTREME_LOCATOR_BASE . "/icons/mapit.gif" . "'/></a><br/>" . __( "Map&Directions", 'xtremelocatorbold' ) . "</div>";
			$map_it2 = ! isset( $_GET['lid'] ) ? "<div class='zoom_it'><a href='#' onClick=\"NewWindow('" . $gconf->domain . "/visitor/googlemap.php?lid=" . $row['_id'] . "&sid=" . $gconf->site_id . "&zoom=14&language=&info=1&width=" . $conf->map_width_details . "%&height=" . $conf->map_height_details . "%', '_blank', " . $conf->map_width_details . ", " . $conf->map_height_details . ", 0);return false;\"><img src='" . XTREME_LOCATOR_BASE . "/icons/glass.gif'/></a></div>" : "";
		}
		if ( $resultType == 2 ) {
			$map = '<div class="' . $css['xtreme_locator_result_item_map']['class'] . '">
       ...
       ' . $map_it2 . '</div>';
		...
		echo "<div class='" . $css['xtreme_locator_result']['class'] . ( ! isset( $_GET['lid'] ) && $conf->location_columns > 1 ? '_columns' . $conf->location_columns : '' ) . "'>" . ( $map_pos == 1 || $map_pos == 2 ? $map : '' ) . ( $map_pos == 2 ? '<div class="xtreme_locator_clear"></div>' : '' ) . "<div class='" . $css['xtreme_locator_result_location']['class'] . ( $resultType == 2 ? '_map' : '' ) . "' " . $style . "/>";
		...
		foreach ( $lok as $k => $v ) {
			...
			if ( $k != '' && $k != 'Id' ) {
				if ( substr( $row[ $k ], 7, 4 ) == "file" ) {
					...
					$row[ $k ] = "<a href=\"" . $gconf->domain . "/common/file.php?id=$row[$k]\">" . __( "Download", 'xtremelocatorbold' ) . "</a>";
				...
				if ( $k == "Distance" ) {
					$row[ $k ] = $row[ $k ] >= 0 ? sprintf( "%.1f", $row[ $k ] ) . " " . __( "Miles", 'xtremelocatorbold' ) : null;
				}
				...
				echo $row[ $k ] != '' ? '<div class="' . $css['xtreme_locator_result_item']['class'] . '"><div id="xtreme_locator_' . $k . '_title" class="' . $css['xtreme_locator_results_title']['class'] . '">' . ( $v['show_title'] == '1' ? $lok[ $k ]['display_name'] : '' ) . '</div> <div id="xtreme_locator_' . $k . '_value" class="' . $css['xtreme_locator_results_value']['class'] . '">' . str_replace( "\\n", "<br>", $row[ $k ] ) . '</div><div class="xtreme_locator_clear"></div></div>' : '';

			}

		}
		echo '</div>' . ( $map_pos == 3 ? '<div class="xtreme_locator_clear"></div>' : '' ) . ( $map_pos == 0 || $map_pos == 3 || $map_pos == 4 ? $map : '' ) . ( $resultType == 1 ? $map_it : "" );
		?>

    </div>
	<?php if ( $c == $conf->location_columns ) {
		echo '<div class="xtreme_locator_clear"></div>';
		$c = 0;
	}
	}
	?>
...
<?php if ( count( $r ) == 0 ) {
	echo '<div id="' . $css['xtreme_locator_search_footer']['class'] . '">' . __( $gconf->not_found, 'xtremelocatorbold' ) . '</div>';
} elseif ( ! isset( $_GET['lid'] ) ) {
	echo '<div id="' . $css['xtreme_locator_search_footer']['class'] . '">' . ( $position > 0 ? "<a href='?q&pos=" . ( $position - $pageSize ) . ( isset( $_GET['search_type'] ) ? "&search_type=" . $_GET['search_type'] : "" ) . ( isset( $_GET['page_id'] ) ? "&page_id=" . $_GET['page_id'] : "" ) . ( isset( $_GET['p'] ) ? "&p=" . $_GET['p'] : "" ) . "'>&lt; " . __( "Previous", 'xtremelocatorbold' ) . "</a> " : "" ) . __( "Records", 'xtremelocatorbold' ) . ' ' . ( $start + 1 ) . " - " . ( $end ) . " (" . count( $r ) . ")" . ( $position + $pageSize < count( $r ) ? "<a href='?q&pos=" . ( $position + $pageSize ) . ( isset( $_GET['search_type'] ) ? "&search_type=" . $_GET['search_type'] : "" ) . ( isset( $_GET['page_id'] ) ? "&page_id=" . $_GET['page_id'] : "" ) . ( isset( $_GET['p'] ) ? "&p=" . $_GET['p'] : "" ) . "'> " . __( "Next", 'xtremelocatorbold' ) . "&gt;</a>" : "" ) . '</div>';
} ?>

<?php if ( isset( $_GET['lid'] ) ) { ?>
    <div id="xtreme_locator_search_back"><a href="javascript:history.back();"><?php echo __( "Back", 'xtremelocatorbold' ); ?></a>
    </div><?php } ?>
</div>
