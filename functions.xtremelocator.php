...
function xtreme_locator_script_enqueuer() {

	wp_enqueue_style( 'xtreme_locator_admin_css', plugin_dir_url( __FILE__ ) . 'css/admin.css' );
	wp_enqueue_style( 'xtreme_locator_public_css', plugin_dir_url( __FILE__ ) . 'css/xtremelocator_pub.css' );
	wp_enqueue_script( 'xtreme_locator_helper_js', plugin_dir_url( __FILE__ ) . 'js/helper-functions.js' );
}


function xtreme_locator_init_admin_menus() {

	add_menu_page( __( "XtremeLocator", 'xtremelocatorbold' ), __( "XtremeLocator", 'xtremelocatorbold' ), "manage_options", "xtreme-locator", "control_panel", XTREME_LOCATOR_BASE . '/icons/icon-16-xtremelocator.png' );
	add_submenu_page( "xtreme-locator", __( "Settings", 'xtremelocatorbold' ), __( "Settings", 'xtremelocatorbold' ), "manage_options", "xtreme-locator-settings", "xtreme_locator_settings" );
	add_submenu_page( 'xtreme-locator', __( "CSS styles", 'xtremelocatorbold' ), __( "Css styles", 'xtremelocatorbold' ), "manage_options", "xtreme-locator-css", 'xtreme_locator_css_styles' );
	add_submenu_page( 'xtreme-locator', __( "Standard search", 'xtremelocatorbold' ), __( "Standard search", 'xtremelocatorbold' ), "manage_options", "xtreme-locator-standard-search", 'xtreme_locator_standard_search' );
	add_submenu_page( 'xtreme-locator', __( "Advanced search", 'xtremelocatorbold' ), __( "Advanced search", 'xtremelocatorbold' ), "manage_options", "xtreme-locator-advanced-search", 'xtreme_locator_advanced_search' );
	add_submenu_page( 'xtreme-locator', __( "Custom search", 'xtremelocatorbold' ), __( "Custom search", 'xtremelocatorbold' ), "manage_options", "xtreme-locator-custom-search", 'xtreme_locator_custom_search' );
	add_submenu_page( 'xtreme-locator', __( "All Locations Map", 'xtremelocatorbold' ), __( "All location map", 'xtremelocatorbold' ), "manage_options", "xtreme-locator-all-locations", 'xtreme_locator_all_location' );
	add_submenu_page( 'xtreme-locator', __( "Location listing", 'xtremelocatorbold' ), __( "Location listing", 'xtremelocatorbold' ), "manage_options", "xtreme-locator-listing", 'xtreme_locator_location_listing' );
	add_submenu_page( 'xtreme-locator', __( "Location admin login", 'xtremelocatorbold' ), __( "Location admin login", 'xtremelocatorbold' ), "manage_options", "xtreme-locator-admin", 'xtreme_locator_location_admin' );

}

function xtreme_locator_uninstall() {
	global $wpdb;
	$wpdb->get_results( "DROP TABLE `" . $wpdb->prefix . "xtremelocator_config`, `" . $wpdb->prefix . "xtremelocator_css`, `" . $wpdb->prefix . "xtremelocator_fields`, `" . $wpdb->prefix . "xtremelocator_layouts`" );
}


function control_panel() {

	include_once( XTREME_LOCATOR_PATH . "/views/control_panel.php" );
}
...
function xtreme_locator_settings() {
	global $wpdb;

	if ( isset( $_POST['field_action'] ) ) {
		if ( $_POST['field_action'] == "save" ) {
			if ( isset( $_POST['xtreme_locator']['id'] ) ) {
				$update_result = $wpdb->get_results( "UPDATE `" . $wpdb->prefix . "xtremelocator_fields` SET `field_id2` = '" . $_POST['xtreme_locator']['field_id2'] . "', field_name='" . $_POST['xtreme_locator']['field_name'] . "' , display_name='" . $_POST['xtreme_locator']['display_name'] . "' WHERE `" . $wpdb->prefix . "xtremelocator_fields`.`id` =" . $_POST['xtreme_locator']['id'] . ";" );
			} else {
				$update_result = $wpdb->get_results( "INSERT INTO`" . $wpdb->prefix . "xtremelocator_fields` (id,field_id2,field_name,enabled,display_name) VALUES (NULL,'" . $_POST['xtreme_locator']['field_id2'] . "', '" . $_POST['xtreme_locator']['field_name'] . "',1,'" . $_POST['xtreme_locator']['display_name'] . "');" );
			}
		}
	}
	...
	if ( ( isset( $_GET['id'] ) || ( isset( $_POST['action'] ) && $_POST['action'] == "add_field" ) ) && ! isset( $_POST['field_action'] ) ) {
		...
		include_once( XTREME_LOCATOR_PATH . "/views/add_field.php" );

	} else {
		if ( count( $_POST ) > 0 && isset( $_POST['action'] ) ) {
			if ( $_POST['action'] == "update" ) {
				$update_result = $wpdb->get_results( "UPDATE `" . $wpdb->prefix . "xtremelocator_config` SET `site_id` = '" . $_POST['xtreme_locator']['site_id'] . "', domain='" . $_POST['xtreme_locator']['domain'] . "', not_found='" . $_POST['xtreme_locator']['not_found'] . "' WHERE `" . $wpdb->prefix . "xtremelocator_config`.`id` =1;" );
		...
		$config = getConfig( 1 );
		$fields = getAllFields();
		include_once( XTREME_LOCATOR_PATH . "/views/settings.php" );
	}
}

function xtreme_locator_get_css() {
	global $wpdb;
...
function xtreme_locator_get_css_array() {
	global $wpdb;
	$css = $wpdb->get_results( "SELECT * FROM `" . $wpdb->prefix . "xtremelocator_css`" );
	$ff  = array();
...
function xtreme_locator_css_styles() {
	global $wpdb;
	//chmod(xtreme_locator_PATH."/xtremelocator_pub.css", 0777);
	if ( count( $_POST ) > 0 ) {
		foreach ( $_POST['xtreme_locator'] as $nm => $css ) {
			$result = $wpdb->get_results( "UPDATE " . $wpdb->prefix . "xtremelocator_css SET `class` = '" . $css . "' WHERE `id` =" . $nm );
		}
		$cssFile = XTREME_LOCATOR_PATH . "/css/xtremelocator_pub.css";
		$fh = fopen( $cssFile, 'w' ) or die( "can't open file" );
		$stringData = $_POST["css_source"];
		fwrite( $fh, $stringData );
		fclose( $fh );
	}
	$css = xtreme_locator_get_css();
	include_once( XTREME_LOCATOR_PATH . "/views/css_styles.php" );
}

function xtreme_locator_standard_search() {
	global $wpdb;
	if ( count( $_POST ) > 0 ) {
	...
	include_once( XTREME_LOCATOR_PATH . "/views/standard_search.php" );
}

function xtreme_locator_advanced_search() {
	global $wpdb;
	...
	include_once( XTREME_LOCATOR_PATH . "/views/advanced_search.php" );
}

function xtreme_locator_custom_search() {
	global $wpdb;
	...
	$afields = getAllFields();
	include_once( XTREME_LOCATOR_PATH . "/views/custom_search.php" );
}

function xtreme_locator_all_location() {
	global $wpdb;
	if ( count( $_POST ) > 0 ) {
	...
	include_once( XTREME_LOCATOR_PATH . "/views/all_location.php" );
}

function xtreme_locator_location_listing() {
	global $wpdb;
	if ( count( $_POST ) > 0 ) {
	...
	$afields = getAllFields();
	include_once( XTREME_LOCATOR_PATH . "/views/location_listing.php" );
}

function xtreme_locator_location_admin() {
	global $wpdb;
	$config = $wpdb->get_results( "SELECT * FROM `" . $wpdb->prefix . "xtremelocator_config` WHERE type=1" );

	include_once( XTREME_LOCATOR_PATH . "/views/location_admin.php" );
}
...
function getLocations( $type ) {
	...
	$searchC    = $config[1];
	$sysFields  = array( 'option', 'view', 'jfcookie', 'Itemid', 'format', 'url', 'url_hash', 'action' );
	$globalC    = $config[0];
	...
	$response = wp_remote_post( $globalC->domain . "/visitor/findLocations.php", $args );
	...
	error_log( 'setting : ' );
	set_transient( xtreme_locator_get_transient_key( 'xtreme_locator_results' ), $r, 60 );
	set_transient( xtreme_locator_get_transient_key( 'pos' ), 0, 60 );
	//$_SESSION["xtreme_locator_results"] = $r;
	//$_SESSION["pos"]        = 0;
	//print_r($_SESSION["xtreme_locator_results"]);
}
...
function includeLocator( $content ) {
	...
	if ( preg_match( '|\[xtreme_locator_standard|', $content ) ) {
		if ( isset( $_GET['search_type'] ) && $_GET['search_type'] == 'advanced' ) {
			$out1 = includeAdvancedSearch();
		} elseif ( isset( $_GET['search_type'] ) && $_GET['search_type'] == 'listing' ) {
			$out1 = includeLocationListing();
		...
		return str_replace( "[xtreme_locator_standard]", $out1, $content );
	} elseif ( preg_match( '|\[xtreme_locator_advanced|', $content ) ) {
		if ( isset( $_GET['search_type'] ) && $_GET['search_type'] == 'listing' ) {
			$out1 = includeLocationListing();
		} else {
			$out1 = includeAdvancedSearch();
		...
		return str_replace( "[xtreme_locator_advanced]", $out1, $content );
	} elseif ( preg_match( '|\[xtreme_locator_custom|', $content ) ) {
		if ( isset( $_GET['search_type'] ) && $_GET['search_type'] == 'listing' ) {
			$out1 = includeLocationListing();
		} else {
			$out1 = includeCustomSearch();
		...
		return str_replace( "[xtreme_locator_custom]", $out1, $content );
	} elseif ( preg_match( '|\[xtreme_locator_listing|', $content ) ) {
		if ( isset( $_GET['search_type'] ) && $_GET['search_type'] == 'advanced' ) {
			$out1 = includeAdvancedSearch();
		} else {
			$out1 = includeLocationListing();
		...
		return str_replace( "[xtreme_locator_listing]", $out1, $content );
	} elseif ( preg_match( '|\[xtreme_locator_all|', $content ) ) {
		if ( isset( $_GET['search_type'] ) && $_GET['search_type'] == 'advanced' ) {
			$out1 = includeAdvancedSearch();
		} elseif ( isset( $_GET['search_type'] ) && $_GET['search_type'] == 'listing' ) {
			$out1 = includeLocationListing();
		...
		return str_replace( "[xtreme_locator_all]", $out1, $content );
	} else {
...
function xtreme_locator_form_parser() {

	if ( isset( $_POST['reset_xtreme_locator_search'] ) ) {
		//error_log('deleting');
		$status = delete_transient( xtreme_locator_get_transient_key( 'xtreme_locator_results' ) );
		//var_dump($status);
	...
	if ( isset( $_REQUEST['reset_xtreme_locator_search'] ) ) {
		error_log( 'deleting xtreme_locator_search' );
		delete_transient( xtreme_locator_get_transient_key( 'xtreme_locator_search' ) );
	}
...
function includeStandardSearch() {
	...
	$css             = xtreme_locator_get_css_array();


	if ( ! isset( $_GET['lid'] ) && ! isset( $_GET['pos'] ) ) {
		if ( isset( $_SESSION["xtreme_locator_search"] ) ) {
			unset( $_SESSION["xtreme_locator_search"] );
		}

	}


	if ( ! isset( $_SESSION["xtreme_locator_search"]['zip'] ) || $_SESSION["xtreme_locator_search"]['zip'] == "" ) {
		if ( isset( $_SESSION["xtreme_locator_search"] ) ) {
			unset( $_SESSION["xtreme_locator_search"] );
		}
	}
	...
	$data = get_transient( xtreme_locator_get_transient_key( 'xtreme_locator_results' ) );

	if ( $data ) {
		if ( ( isset( $_GET['lid'] ) && $conf->show_form_details == 1 ) || ( ! isset( $_GET['lid'] ) && $conf->show_form_list == 1 ) ) {
			require( XTREME_LOCATOR_PATH . "/pub/standard_search_form.php" );
		}
		require( XTREME_LOCATOR_PATH . "/pub/standard_search_results.php" );
	} else {
		require( XTREME_LOCATOR_PATH . "/pub/standard_search_form.php" );
	}
...
function includeAdvancedSearch() {
	...
	if ( $data ) {
		if ( ( isset( $_GET['lid'] ) && $conf->show_form_details == 1 ) || ( ! isset( $_GET['lid'] ) && $conf->show_form_list == 1 ) ) {
			require( XTREME_LOCATOR_PATH . "/pub/advanced_search_form.php" );
		}
		require( XTREME_LOCATOR_PATH . "/pub/advanced_search_results.php" );
	} else {
		require( XTREME_LOCATOR_PATH . "/pub/advanced_search_form.php" );
	}
	$out1 = ob_get_contents();
...
function includeCustomSearch() {
	...
	if ( $data ) {
		if ( ( isset( $_GET['lid'] ) && $conf->show_form_details == 1 ) || ( ! isset( $_GET['lid'] ) && $conf->show_form_list == 1 ) ) {
			require( XTREME_LOCATOR_PATH . "/pub/custom_search_form.php" );
		}
		require( XTREME_LOCATOR_PATH . "/pub/custom_search_results.php" );
	} else {
		require( XTREME_LOCATOR_PATH . "/pub/custom_search_form.php" );
	}
	$out1 = ob_get_contents();
...
function includeLocationListing() {
	...
	getLocations( 3 );
	ob_start();
	require( XTREME_LOCATOR_PATH . "/pub/location_listing.php" );
	$out1 = ob_get_contents();
	ob_end_clean();
...
function includeAllMap() {
	...
	getLocations( 5 );
	ob_start();
	require( XTREME_LOCATOR_PATH . "/pub/all_map.php" );
	$out1 = ob_get_contents();
	ob_end_clean();
...
function xtreme_locator_register_widgets() {
	global $wpdb;
	wp_register_sidebar_widget( "10", __( "XtremeLocator search box", 'xtremelocatorbold' ), 'xtreme_locator_search_widget', array( "classname" ) );
	wp_register_widget_control( "10", __( "XtremeLocator search box", 'xtremelocatorbold' ), "xtreme_locator_standard_search_widget_init", 300, 300 );

}

function xtreme_locator_search_widget() {
	global $wpdb;
	$url         = get_option( 'xtreme_locator_widget_url' );
	$type        = get_option( 'xtreme_locator_search_type' );
	$title       = get_option( 'xtreme_locator_widget_title' );
	$description = get_option( 'xtreme_locator_widget_desc' );
	$zip         = get_option( 'xtreme_locator_widget_zip' );
	echo '<aside id="xtreme_locator_pub_widget" class="widget  widget-container widget_xtreme_locator"><h2 class="widget-title">' . $title . '</h2>';
	echo '<p id="xtreme_locator_pub_widget_description">' . $description . '</p>';
	...
	if ( $type == 1 ) {
		...
		echo '<script type="text/javascript" > 
			function checkZipWidget(){									
					zip=document.getElementById("xtreme_locator_pub_widget_zip");
					city=document.getElementById("xtreme_locator_pub_widget_city");
					if(isNaN(zip.value)){
		...
		echo '<form action="' . XTREME_LOCATOR_ADMIN_POST_URL . '" method="post" id="xtreme_locator_pub_widget_form" onSubmit="checkZipWidget()">';
		if ( $conf->search_type == 0 ) {
			echo ' <div><label for="xtreme_locator_search_country" id="xtreme_locator_search_country_label">' . __( "Country", 'xtremelocatorbold' ) . ':</label><select id="XlocatorCountry" name="country" onChange="checkZip(this);">
          </select></div>';
		}
		echo '<div><label for="xtreme_locator_pub_widget_zip" id="xtreme_locator_pub_widget_zip_label">' . __( $zip, 'xtremelocatorbold' ) . ':</label><input type="text" id="XlocatorZip" class="xtreme_locator_pub_widget_zip" name="zip"> <input type="hidden" name="city" id="xtreme_locator_pub_widget_city"  value=""></div>';
		if ( $conf->search_type == 0 ) {
		...
		if ( $conf->search_type == 2 ) {
			echo '<div><label for="xtreme_locator_pub_search_distance" id="xtreme_locator_pub_search_distance_label" >' . __( "Distance", 'xtremelocatorbold' ) . ':</label><input type="text" name="distance" id="xtreme_locator_search_distance" value="' . isset( $_SESSION["xtreme_locator_search"]['distance'] ) ? $_SESSION["xtreme_locator_search"]['distance'] : "" . '" name="distance"/></div>';
		}
		echo '<input type="submit" value="' . __( "Search", 'xtremelocatorbold' ) . '" id="xtreme_locator_pub_search_submit"></form>';
	} else {
		$config = $wpdb->get_results( "SELECT * FROM `" . $wpdb->prefix . "xtremelocator_config` WHERE type=3" );
		$conf   = $config[0];
		echo '<div id="xtreme_locator_pub_advanced_form_code">' . $conf->form_code . '</div>';
	}
	echo '</aside>';
}

function xtreme_locator_standard_search_widget_init() {
	global $wpdb;
	$url         = get_option( 'xtreme_locator_widget_url' );
	$type        = get_option( 'xtreme_locator_search_type' );
	$title       = get_option( 'xtreme_locator_widget_title' );
	...
	$zip         = get_option( 'xtreme_locator_widget_zip' );
	if ( count( $_POST ) > 0 ) {
		if ( isset( $_POST['xtreme_locator_widget_url'] ) ) {
			update_option( 'xtreme_locator_widget_url', $_POST['xtreme_locator_widget_url'] );
		}
		if ( isset( $_POST['xtreme_locator_search_type'] ) ) {
			update_option( 'xtreme_locator_search_type', $_POST['xtreme_locator_search_type'] );
		}
		if ( isset( $_POST['xtreme_locator_widget_title'] ) ) {
			update_option( 'xtreme_locator_widget_title', $_POST['xtreme_locator_widget_title'] );
		}
		if ( isset( $_POST['xtreme_locator_widget_desc'] ) ) {
			update_option( 'xtreme_locator_widget_desc', $_POST['xtreme_locator_widget_desc'] );
		}
		if ( isset( $_POST['xtreme_locator_widget_zip'] ) ) {
			update_option( 'xtreme_locator_widget_zip', $_POST['xtreme_locator_widget_zip'] );
		}
	}
	echo __( "Search type", 'xtremelocatorbold' ) . ' <select name="xtreme_locator_search_type"><option value="1" ' . ( $type == 1 ? 'selected="selected"' : "" ) . '>' . __( "Standard search", 'xtremelocatorbold' ) . '</option><option value="2" ' . ( $type == 2 ? 'selected="selected"' : "" ) . '>' . __( "Advanced search", 'xtremelocatorbold' ) . '</option></select><br/>';
	echo __( "Full search URL", 'xtremelocatorbold' ) . ' <br/><input type="name" name="xtreme_locator_widget_url" value="' . $url . '"/><br/>';
	echo __( "Box title", 'xtremelocatorbold' ) . ' <br/><input type="name" name="xtreme_locator_widget_title" value="' . $title . '"/><br/>';
	echo __( "Description", 'xtremelocatorbold' ) . ' <br/><textarea name="xtreme_locator_widget_desc" cols="20" rows="5">' . $description . '</textarea><br/>';
	echo __( "Zip title", 'xtremelocatorbold' ) . ' <br/><textarea name="xtreme_locator_widget_zip" cols="20" rows="5">' . $zip . '</textarea>';
}

function checkLocatorTag( $content ) {
	$url = get_option( 'xtreme_locator_widget_url' );
	//if($url==""){
	if ( preg_match( '|\[xtreme_locator_standard|', $content ) || preg_match( '|\[xtreme_locator_advanced|', $content ) ) {
		update_option( 'xtreme_locator_widget_url', get_permalink( $_POST['post_ID'] ) );
	}
...
function xtreme_locator_install() {
	global $wpdb;
...
SQL;
	$url   = XTREME_LOCATOR_ADMIN_POST_URL;
...
INSERT INTO $css_table_name (id, name, class) VALUES
	(1, 'xtreme_locator_search_form', 'xtreme_locator_search_form'),
	(2, 'xtreme_locator_form_message', 'xtreme_locator_form_message'),
	(3, 'searchForm', 'searchForm'),
	(5, 'xtreme_locator_wraper', 'xtreme_locator_wraper'),
	(6, 'xtreme_locator_advanced_search_link', 'xtreme_locator_advanced_search_link'),
	(7, 'xtreme_locator_all_locations_link', 'xtreme_locator_all_locations_link'),
	(8, 'xtreme_locator_search_results', 'xtreme_locator_search_results'),
	(9, 'xtreme_locator_search_locations', 'xtreme_locator_search_locations'),
	(10, 'xtreme_locator_result', 'xtreme_locator_result'),
	(11, 'xtreme_locator_result_item_map', 'xtreme_locator_result_item_map'),
	(12, 'xtreme_locator_result_location', 'xtreme_locator_result_location'),
	(13, 'xtreme_locator_result_item', 'xtreme_locator_result_item'),
	(14, 'xtreme_locator_search_footer', 'xtreme_locator_search_footer'),
	(15, 'xtreme_locator_results_title', 'xtreme_locator_results_title'),
	(16, 'xtreme_locator_results_value', 'xtreme_locator_results_value');
SQL;
...
SQL;
	...
	add_option( 'xtreme_locator_widget_url', '' );
	add_option( 'xtreme_locator_search_type', 1 );
	add_option( 'xtreme_locator_widget_title', "Dealer locator" );
	add_option( 'xtreme_locator_widget_zip', "Zip" );
	add_option( 'xtreme_locator_widget_desc', "Enter locator zip" );
}

function xtreme_locator_prevent_upgrade( $opt ) {
	global $update_class;
...
function xtreme_locator_get_transient_key( $name ) {
